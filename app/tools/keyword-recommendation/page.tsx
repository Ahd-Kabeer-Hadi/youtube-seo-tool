...
const KeywordRecommendation: React.FC = () => {
      ...
      videoDetails.forEach((video) => {
        if (video && video.snippet.tags) {
          ...
          tags.forEach((tag) => {
            ...
            if (fuzzyMatch(normalizedTag, title.toLowerCase()) || fuzzyMatch(normalizedTag, description.toLowerCase())) {
  ...
  const fuzzyMatch = (str1: string, str2: string) => {
    const words = str1.split(" ").map((word) => word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
    ...
    <div className="min-h-screen w-full dark:bg-black bg-white relative flex items-center justify-center">
      ...
      {status === "authenticated" ? (
        <div className="flex flex-col gap-4 justify-center items-center max-w-7xl mx-auto p-4">
          <h1 className="text-3xl font-bold relative z-20 py-8">The Best Youtube Keyword Research Tool</h1>
          <form onSubmit={handleSubmit} className="w-full max-w-lg">
            ...
            <button
              type="submit"
              className="mt-4 w-full p-2 bg-blue-500 text-white font-semibold rounded-md shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              Search
            </button>
          </form>
          {isLoading && <div className="text-center text-neutral-500 mt-4">Loading...</div>}
          {apiResponse && (
            <div className="mt-8 w-full max-w-4xl rounded-lg bg-gradient-to-b from-neutral-950-100 to-stone-950 p-4">
              <h2 className="text-xl font-semibold mb-4">Top Related Keywords for: {keyword}</h2>
              <div className="flex flex-wrap justify-center">
                {keywords.length > 0 ? (
                  keywords.map((item, index) => (
                    <span key={index} className="m-1 p-2 bg-gray-200 rounded-md shadow-sm text-sm font-medium text-gray-700">
                      {item.keyword}
                    </span>
                  ))
                ) : (
                  <div className="text-center text-neutral-500">It seems the keyword is not that popular, try with an alternative keyword.</div>
                )}
              </div>
            </div>
          )}
        ...
        <div>You are not signed in and redirecting to sign-in page, please sign in and try again.</div>
      )}
    </div>
  );
};
